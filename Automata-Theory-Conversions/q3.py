...
def get_pred_succ(state, input_symbols):
    ...
    successors = []
    curr_dict_for_from = {
        st: {to: v for to, v in val.items() if to == state}
        for st, val in input_symbols.items()
    }
...
def DFA_to_regex(input_symbols, state_intitial, state_final):
    ...
    for state in dfa["states"]:
                ...
                for successor in successors:
                    if successor in input_symbols[predecessor].keys():
                        ...
                        if input_symbols[predecessor][successor] != "":
                            pre_suc_input_exp = (
                                "(" + input_symbols[predecessor][successor] + ")"
                            )

                        if check_self_loop(state):
                            self_loop_input_exp = (
                                "(" + input_symbols[state][state] + ")" + "*"
                            )

                        if input_symbols[predecessor][state] != "":
                            from_pre_input_exp = (
                                "(" + input_symbols[predecessor][state] + ")"
                            )

                        if input_symbols[state][successor] != "":
                            to_suc_input_exp = (
                                "(" + input_symbols[state][successor] + ")"
                            )
        ...
        # remove the state
        input_symbols = {
            st: {to: v for to, v in inp.items() if to != state}
            for st, inp in input_symbols.items()
            if st != state
        }
...
if __name__ == "__main__":
    ...
    reg_exp = DFA_to_regex(
        input_symbols, dfa["start_states"][0], dfa["final_states"][0]
    )
    out_regex(reg_exp)
